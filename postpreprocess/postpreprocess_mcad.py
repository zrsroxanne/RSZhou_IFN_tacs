...
def preprocess_volume_for_alff_reho(
    ...
    for subject in tqdm(subject_list, desc="postpreprocessing"):
        try:
            bold_file = os.path.join(
                fmriprep_dir, subject, 'ses-1/func',
                f'{subject}_ses-1_task-rest_run-0_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz'
            )
            mask_file = bold_file.replace('_desc-preproc_bold.nii.gz', '_desc-brain_mask.nii.gz')
            
            if not os.path.exists(bold_file) or not os.path.exists(mask_file):
                errors.append(f"{subject}: 文件不存在")
                continue
            
            sub_dir = os.path.join(output_dir, subject)
            os.makedirs(sub_dir, exist_ok=True)
            output_file = os.path.join(sub_dir, f'{subject}_preprocessed_gsr_MNI2009_2mm.nii.gz')
            ...
            if use_gsr:
                confounds, _ = load_confounds(
                    ...
                    global_signal='derivatives', demean=False
                )
            else:
                confounds, _ = load_confounds(
                    bold_file,
                    strategy=['motion', 'wm_csf'],
                    motion='derivatives', wm_csf='derivatives', demean=False
                )
            n = confounds.shape[0]
            ...
            if np.any(np.isnan(confounds.values)):
                errors.append(f"{subject}: Confounds exist NaN")
                continue
    ...
    print(f"\finish: {len(processed_files)}/{len(subject_list)} subjects")
...
if __name__ == "__main__":
    ...
    preprocess_volume_for_alff_reho(
        fmriprep_dir='/data1/temp_data/disk2/Dataset/MCAD/fmriprep/fmriprep_2314',
        output_dir='/data1/rszhou/MCAD/fmriprep/processed/clean_imags/volume',
        info_table_path='/data/home/rszhou/xw_project_zhou/infoTable/MCAD_info_809_fmriprep_withTIV_ts_length_new.csv',
        use_gsr=True,
        sub_name_idx=8
    )
    ...
    #     fmriprep_dir='/data1/temp_data/disk2/Dataset/MCAD/fmriprep/fmriprep_2314',
    #     save_dir='/data1/rszhou/MCAD/fmriprep/processed/clean_imags/postprocess_fs5',
    #     sub_pattern='sub-*',
    #     use_gsr=True,
    #     smooth_fwhm=4,
    #     sub_name_idx=8
